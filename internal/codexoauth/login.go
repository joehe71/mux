...
func Login(ctx context.Context, onReturn func()) (Credentials, error) {
	...
	server.Handler = callbackHandler(state, resultCh, returnCh, onReturn)
...
func callbackHandler(expectedState string, resultCh chan<- callbackResult, returnCh chan<- struct{}, onReturn func()) http.Handler {
	...
	mux.HandleFunc("/auth/return", func(w http.ResponseWriter, _ *http.Request) {
...
<body>
  ...
  <button type="button" onclick="window.location.href='/auth/return'">返回 Mux</button>
</body>
</html>`)
}
